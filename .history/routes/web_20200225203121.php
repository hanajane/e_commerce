...
//related products
// Route::get('/singleProduct/{id}', ["uses" => "ProductsController@showRelatedProducts", 'as'=>'relatedProducts']);
...
//test images
Route::get('testImage', ["uses" => "StoreImageController@index", 'as'=>'testImage']);
...
Route::get('/testStorage', function()
    {
        // return "<img src=".Storage::url('Images/jacket.jpg').">";    // when using the ::url function you dont have to specify the whole path. just the single folder where the file is. it knows the image is inside the public directory
        // return Storage::disk('local')->url('Images/jacket.jpg');     // when using class Storage with the function of ::disk, it also knows the file is inside the public directory but no <img> tag, not an image just the url
        // print_r(Storage::disk("local")->exists("public/Images/jacket.jpg"));     //exists method, need to specify the complete file path/url because it doesnt know where the file exactly sitting in. checks if the file exists or not. boolean
        // Storage::delete('public/Images/jacket.jpg');                 //delete, deletes the file. like exists method, you have to specify the complete path/url. remember that the storage folder thats inside the public folder is originally copied link from the storage/app/public/Images
    });
